...
let BattleRoom = (() => {
	...
	BattleRoom.prototype.logBattle = function (p1score, p1rating, p2rating) {
		...
		const date = new Date();
		const logsubfolder = date.toLocaleDateString('jp');
...
let ChatRoom = (() => {
	function ChatRoom(roomid, title, options) {
		...
		if (Config.logchat) {
			...
			this.logEntry = function (entry, date) {
				const timestamp = new Date().toLocaleDateString('jp', {hour: '2-digit', minute: '2-digit', second: '2-digit'}).split(' ')[1];
	...
	ChatRoom.prototype.rollLogFile = function (sync) {
		...
		mkdir(basepath, '0755', () => {
			const dateString = date.toLocaleDateString('jp');
